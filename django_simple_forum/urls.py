...
urlpatterns = [
    ...
    url(r'^topic/like/(?P<slug>[-\w]+)/$', csrf_exempt(views.TopicLike.as_view()), name="like_topic"),
    ...
    url(r'^comment/delete/(?P<comment_id>[-\w]+)/$',
        csrf_exempt(views.CommentDelete.as_view()), name="comment_delete"),
    ...
    url(r'^tags/$', csrf_exempt(views.ForumTagsList.as_view()), name="forum_tags"),
    url(r'^badges/$', csrf_exempt(views.ForumBadgeList.as_view()), name="forum_badges"),
    url(r'^profile/$', views.UserProfileView.as_view(), name="user_profile"),
    url(r'^upload/profile-pic/$', csrf_exempt(views.UserProfilePicView.as_view()), name="user_profile_pic"),
    url(r'^send-mail/settings/$', csrf_exempt(views.UserSettingsView.as_view()), name="user_settings"),
    ...
    url(r'^dashboard/category/list/$', csrf_exempt(views.CategoryList.as_view()), name="categories"),
    url(r'^dashboard/category/add/$', views.CategoryAdd.as_view(), name="add_category"),
    url(r'^dashboard/category/delete/(?P<slug>[-\w]+)/$',
        csrf_exempt(views.CategoryDelete.as_view()), name="delete_category"),
    ...
    url(r'^dashboard/badge/list/$', csrf_exempt(views.BadgeList.as_view()), name="badges"),
    url(r'^dashboard/badge/add/$', views.BadgeAdd.as_view(), name="add_badge"),
    url(r'^dashboard/badge/delete/(?P<slug>[-\w]+)/$', csrf_exempt(views.BadgeDelete.as_view()), name="delete_badge"),
    ...
    url(r'^dashboard/users/list/$', csrf_exempt(views.UserList.as_view()), name="users"),
    url(r'^dashboard/users/delete/(?P<user_id>[a-zA-Z0-9_-]+.*?)/$',
        csrf_exempt(views.DashboardUserDelete.as_view()), name="delete_user"),
    url(r'^dashboard/users/status/(?P<user_id>[a-zA-Z0-9_-]+.*?)/$',
        csrf_exempt(views.UserStatus.as_view()), name="user_status"),
